...
name: eric-bridge
description: |
  Agent 基建：把任意 skill 或 skill 集合目录桥接到 Claude Code（~/.claude/skills）、Codex（~/.codex/skills）、WorkBuddy（~/.workbuddy/skills）和 Grok（~/.grok/skills）。支持 eric 仓库内 skill，也支持外部 skill 绝对路径。Claude / Codex / WorkBuddy 使用软链，Grok 生成带 user_invocable: true 的薄 bridge。默认不写 ~/.agents/skills，避免 Codex 同时扫描 ~/.codex/skills 和 ~/.agents/skills 时重复显示；只有用户明确需要豆包 Mac App / Trae Solo 等通用 Agents 时才使用 --with-agents。
  触发方式：/eric-bridge、/bridge、「桥接这个 skill」「桥接整个 skills 目录」「把这个 skill 接到 Claude Code、Codex、WorkBuddy 和 Grok」「让多个 Agent 都能调用这个 skill」「给豆包/Trae 也桥接」「取消桥接」「查看桥接状态」
  Bridge any skill folder or directory of skill folders to Claude Code, Codex, WorkBuddy, and Grok using symlinks plus Grok bridge files. Supports eric skills and external skill directories. By default it avoids ~/.agents/skills because Codex also reads that directory and duplicate names will appear. Use --with-agents only for generic Agents such as Doubao or Trae Solo.
  Trigger: /eric-bridge, /bridge, "bridge this skill", "bridge this skills directory", "make this skill available to Claude Code, Codex, WorkBuddy, and Grok", "also bridge to Doubao or Trae", "unlink this skill", "check bridge status"
---

# eric-bridge：多端 skill 桥接
...
这样 Claude Code、Codex、WorkBuddy、Grok 等 Agent 都能通过同一个源目录调用该 skill。源目录改动后，各端自动同步。源目录可以在 eric 仓库内，也可以在外部项目、`~/.claude/skills` / `~/.codex/skills` / `~/.workbuddy/skills` / `~/.grok/skills` 以外的本地目录、iCloud 目录或其他工作区。
...
- skill 名称：`eric-hook`
- 相对路径：`skills/eric-hook`
- 绝对路径：`/Users/.../eric/skills/eric-hook`
- 外部绝对路径：`/Users/.../.agents/skills/lark-doc`
- skill 集合目录：`/Users/.../eric/skills`
- 当前上下文刚创建或刚修改的 skill

按优先级判断：

1. 用户明确给了绝对路径，直接使用该路径。
2. 用户给了相对路径，先按当前工作目录解析，再按 eric 仓库根目录解析。
3. 用户只给 skill 名称，优先查当前工作目录下的同名目录，再查 eric 仓库 `skills/<name>`。
...
```bash
skills/eric-bridge/scripts/bridge-skill.sh link <skill-name-or-path>
```

例子：

```bash
skills/eric-bridge/scripts/bridge-skill.sh link eric-hook
skills/eric-bridge/scripts/bridge-skill.sh link skills/eric-beta-framework-builder
skills/eric-bridge/scripts/bridge-skill.sh link skills
skills/eric-bridge/scripts/bridge-skill.sh link "/absolute/path/to/skill"
skills/eric-bridge/scripts/bridge-skill.sh link "/Users/me/external-skills/my-skill"
skills/eric-bridge/scripts/bridge-skill.sh link "/Users/me/external-skills"
```
...
```bash
skills/eric-bridge/scripts/bridge-skill.sh link --with-agents <skill-name-or-path>
```
...
```bash
skills/eric-bridge/scripts/bridge-skill.sh status <skill-name-or-path>
```
...
```bash
skills/eric-bridge/scripts/bridge-skill.sh unlink <skill-name-or-path>
```
...
- 不能删除源目录；
- 不能把 `skills/eric-bridge` 自身复制到各端；Grok 只能生成薄 bridge。
