...
description: |
  ...
  触发方式：/eric-skill-cleaner、/清理 skill、/检查 skill、「扫描本地 skill」「检测 skill 广告」「清除有问题的 skill」「审查我的 skill」
  ...
  Trigger: /eric-skill-cleaner, /clean skills, /check skills, "scan my local skills", "detect skill ads", "clean problematic skills"
---

# eric-skill-cleaner：本地 skill 清理器
...
```bash
python3 skills/eric-skill-cleaner/scripts/skill_cleaner.py scan
```
...
```bash
python3 skills/eric-skill-cleaner/scripts/skill_cleaner.py scan --root "/absolute/path/to/skills"
python3 skills/eric-skill-cleaner/scripts/skill_cleaner.py scan --root "/path/a" --root "/path/b"
```
...
```bash
python3 skills/eric-skill-cleaner/scripts/skill_cleaner.py quarantine "/absolute/path/to/skill" --yes --reason "用户确认：<原因>"
```
...
```bash
python3 skills/eric-skill-cleaner/scripts/skill_cleaner.py list-quarantine
```

确认来源与恢复目标均无冲突后运行：

```bash
python3 skills/eric-skill-cleaner/scripts/skill_cleaner.py restore "/absolute/path/to/quarantined/skill" "/absolute/path/to/target" --yes
```
