...
def command_scan(args: argparse.Namespace) -> int:
    ...
    skills = [path for path in skills if path.name != "eric-skill-cleaner"]
...
def command_quarantine(args: argparse.Namespace) -> int:
    ...
    if not (skill_dir / "SKILL.md").is_file() or skill_dir.name == "eric-skill-cleaner":
        print("拒绝执行：目标必须是非 eric-skill-cleaner 的 skill 目录。", file=sys.stderr); return 2
