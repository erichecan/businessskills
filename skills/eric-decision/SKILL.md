...
description: |
  ...
  触发方式：/eric-decision、/决策系统、/决策立案、/结果回填、/状态画像
  ...
  Trigger: /eric-decision, /决策系统, /决策立案, /结果回填, /状态画像
---

# eric-decision：个人决策系统
...
### 模式 A：初始化（`/eric-decision` 在空目录或新域第一次触发）
...
5. 返回一句：`决策项目已建立：{路径}。下次直接说 /eric-decision 或 /状态画像 / /决策立案 / /结果回填 继续。`

### 模式 B：更新当前状态（默认模式）

触发：用户说"最近变化是""现在卡在""我现在的状态是"，或直接 `/eric-decision`。
...
|---|---|
| 用户连目标都说不清 | `/eric-goal` |
| 用户没把问题边界划清 | `/eric-good-question` |
| 用户在问这个方向值不值得做 | `/eric-diagnosis` |
| 用户想找参考路径 | `/eric-benchmark` |
| 用户知道该做但做不动 | `/eric-action` |
| 用户在拆某个具体概念到底什么意思 | `/eric-deconstruct` |

eric-decision 处理的是**已经在做、要长期跟踪的领域**。不要替代上面这些 skill 的入口。
