...
description: |
  ...
  触发方式：/eric-resonate、「这个文稿有没有戳中人」「帮我看看能不能发」「这个完播率会不会低」「文稿发出去有没有流量」
  ...
  Trigger: /eric-resonate, "will this resonate", "check my draft", "will people watch this"
---

# eric-resonate：文稿共鸣诊断
...
**不做**：
- 预测平台算法表现（完播率、点击率数字）——超出本 skill 边界。保留用户材料，交回 `/eric` 判断专项入口
- 检查结构是否完整（是否缺少什么部分）—— 那是另一类问题
- 分析已验证的内容为什么能火——属于相邻能力范围。保留用户材料，交回 `/eric` 判断入口
