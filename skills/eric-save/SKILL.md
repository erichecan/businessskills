...
description: |
  ...
  触发方式：/eric-save、/存档、「保存这次诊断」「记下来」「这个结论留着」
  Save the current diagnosis state to disk for cross-session recall.
  Trigger: /eric-save, "save this diagnosis", "remember this"
---
...
> 现在没什么可存的——前面没做过诊断。先用 `/eric-diagnosis` 或别的诊断 skill 走一轮，再来存档。
...
如果对话里推荐了下一步 skill（比如 eric-diagnosis 末尾推荐了 `/eric-benchmark`），就填上。
如果没有明显推荐，留空。
...
| 触发条件 | 推荐话术 |
|---|---|
| 用户存完存档，且当前项目下已有 ≥3 份 | 「这个项目下已经攒了 {N} 份存档。如果你想看个总览，输入 `/eric-report` 出一份合并报告。」 |
| 用户存完，但 next_skill 字段非空 | 「下次回来直接走 `/eric-restore`，能直接接到 {next_skill}。」 |
