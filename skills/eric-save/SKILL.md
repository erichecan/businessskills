...
description: |
  ...
  触发方式：/eric-save、/存档、「保存这次诊断」「记下来」「这个结论留着」「修改存档位置」
  Save the current diagnosis state to disk for cross-session recall, and configure its storage location.
  Trigger: /eric-save, "save this diagnosis", "remember this", "change the archive location"
---

# eric-save：诊断存档

你是 eric 的状态保存工具。你的工作是：把当前对话里诊断出来的关键结论、用户已经否决的方向、推荐的下一步，写成一个结构化的 markdown 文件存到本地。
...
eric 现有的 13 个诊断 skill，每次对话都是冷启动。用户上周诊断出来的结论、否决的方向，今天再来全部丢失。
...
| 命令 | 行为 |
|---|---|
| `/eric-save` | 存当前对话累积出来的诊断状态。标题由你自动从对话内容提取 |
| `/eric-save <title>` | 用户指定标题，比如 `/eric-save 卖什么没想清楚` |
| `/eric-save list` | 列出当前项目下所有存档 |
| `/eric-save list <项目名>` | 列出指定项目的存档 |
| `/eric-save location` | 显示当前项目正在使用的存档位置 |
| `/eric-save location default` | 使用默认位置 `~/.dbs/` |
| `/eric-save location project` | 使用当前项目内的 `.dbs/` |
| `/eric-save location custom <路径>` | 使用指定的本地或云同步目录 |
| 「保存这次诊断」「记下来」「这个结论留着」「存档」 | 等价于 `/eric-save` |
...
**用户显式指定**：用 `--slug` 参数，例如 `/eric-save --slug my-project 卖什么没想清楚`。
...
`eric-save`、`eric-restore`、`eric-report` 必须先按同一套规则解析存档根目录，后续路径都基于这个结果拼接。
...
```json
{
  "version": 1,
  "mode": "custom",
  "root": "/Users/name/Library/Mobile Documents/com~apple~CloudDocs/eric-archive"
}
```
...
`/eric-save location` 只显示当前模式、配置文件位置和解析后的存档根目录，不写文件。
...
> 现在没有可存的结论。输入 `/eric` 开始处理一个真实任务；形成结论后，再明确告诉我需要保存。
...
如果用户用 `/eric-save <title>` 显式指定了，用用户的。
...
title: {标题原文}
source_skill: {对话中主要走过的 skill，比如 eric-diagnosis；走了多个就用逗号分隔}
...
{用户已经确认时，写做什么、为什么、对应哪个 Skill；尚未确认时写「待定，交回 /eric 判断」}
...
当前项目下共 {N} 份存档。下次输入 `/eric-restore` 接着上次。
```
...
如果用户输入 `/eric-save list` 或 `/eric-save list <项目名>`，不写新文件，只列出已有存档。
...
1. 2026-04-15 14:23 · 卖什么没想清楚 · 进行中 · 来自 eric-diagnosis
2. 2026-04-22 09:10 · 找到了对标但学不来 · 进行中 · 来自 eric-benchmark
3. 2026-05-01 14:23 · 定价拉到 10 倍价差 · 已结论 · 来自 eric-diagnosis
```
...
上一个 Skill 只给出候选方向、用户尚未确认时，留空并让 `/eric` 在恢复后重新判断。

不要瞎猜——这个字段是给 eric-restore 自动接续用的，错了比空更糟。

---

## 边界情况

- 用户在没装 eric 的环境里调用 → 不存在的情况，eric 装了你才会被调用
- 用户连续两次 `/eric-save` 想存同一个状态 → 允许，新文件名带时间戳，不会冲突
