...
LANE_HINT = {
    ...
    "祝福流": ("⚠️ 本次按**祝福流**口径审核（系列二「祝福 + 锦囊」），报告开头须注明。"
               "这条 lane **不吃搜索流量**，所以：搜索意图、标题关键词是否最左、"
               "首图是否搜索原句、搜索位前排强度 —— 这四项**本次一律不适用，不得据此扣分或降级**。"
               "改判这七条：① 开头是不是「刷到这条，做什么都能成功，我祝你在[场景]，[诉求]」"
               "这个固定句式（2026-09-09 定；先给一句不挂靠场景的确定成功宣称，再收窄到具体场景，"
               "缺了泛好运宣称、或直接从「我祝你」开头 = 不成立）；"
               "② 祝福和锦囊之间有没有夹一段「痛点共鸣」（2026-09-09 定，同日二次修正）："
               "必须是**直接问诊式**（「你是不是也……」／「是不是每次……就……」，把读者当下真实"
               "状态直接问出来），⛔「如果没刷到这条祝福，你现在可能正在经历……」这种反事实假设"
               "算不成立——没人会这么说话；评论区原话如果有就佐证在这一段里——"
               "开头一祝福完就直接甩锦囊话术、没有这段情绪铺垫 = 判返工；"
               "③ 祝福后面有没有跟一个**具体场景的锦囊**，锦囊是不是一句可直接照抄的话术或当场能用的判据"
               "（「要自信一点」这种不算）；④ 概念点名措辞是否逐字照术语库；"
               "⑤ 标题是否人话——数一遍标题里有几个独立信息点（场景/动作A/动作B/理由），"
               "超过两个就算堆砌超载（2026-09-08「祝你被同事打听隐私时不交代也不树敌」是反面案例），"
               "命中直接判返工；⑥ CTA 除了回字母有没有再留一个「转发传好运」的具体动作"
               "（要点名一个具体的人，「觉得有用就转发」这种空泛号召不算，缺了判返工）；"
               "⑦ 有没有编造原话 / 承诺结果 / 身份头衔。"
               "红线沿用：编造、冒充、承诺结果、身份头衔、出现「表达力」三字。"),
}
...
维度列按 skill 的评分卡顺序填分（**2026-09-16 起的新权重**）：
搜索意图 35 / 标题 20 / 首图 5 / 开头 5 / 正文 30 / CTA 5，
**六项合计必须等于总分，且总分上限 100**。
⚠️ 新卡的重心是「读者带走了什么」：正文从 10 分升到 30 分（本卡最重），
标题从 35 降到 20，首图从 10 降到 5。按新权重打分，别沿用旧比例。
