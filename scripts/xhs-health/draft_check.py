...
# ⛔ 2026-09-17 删除 CTA_HINTS / CTA_OPTION / CTA_TAIL_CHARS 三个常量：
# CTA 降为可选之后，「有没有互动段」「有没有编号选项」两条检查都已取消
# （理由见 check_one 里那段）。留着就是死常量，下次有人会以为它们还在生效。
# 仍在用的只有 CTA_EXPENSIVE —— 它拦的是对读者有真实风险的那种 CTA，不是形态要求。

# 「说说你的情况」型的特征词。命中即判贵 —— 这类要读者写一段、还要公开贴原话。
# ⛔ 这条不随 CTA 降级一起放宽：它拦的不是形态，是**对读者的真实风险** ——
# 话题是被孤立/不想转正/下属不服管，公开贴出领导原话在真实职场里会出事。
...
# 阈值也不同，各自独立配置，不共享判据）。
#
# ⛔ 2026-09-16 阈值 0.30 → 0.50。**算法没问题，是当初那条线划在了错的位置。**
#
# 拿两组已知答案实测（不是估的，是逐篇核过的）：
#   · 7 篇确定误伤（各自的关键词一篇都没发过，`published_keyword_counts()` 全 0）
#     —— 最高分 39%（35/36/36/37/38/39/39）
#   · 7 篇确定真重复（含 5 篇一字不差的）—— 最低分 69%（69/69/100×5）
# 中间 30 个百分点没有任何样本，而 0.30 恰好压在误伤那一带的上沿。
# 也试过「最长公共连续子串占比」这种更聪明的度量，分离间隙反而只剩 2 个点，
# 比现在的算法更差 —— 所以不换算法，只把线挪到空走廊中间。
#
# 当初为什么会划错：09-05 定这条线时，拿来当负样本的是**随机不相关的两条标题**
# （实测 0.13-0.16）。但真实的负样本不是随机标题，是「**不同话题、但标题写手
# 用了 eric-xhs-audit 维度 2 里那批实测高分模板**」—— 这批落在 30-40%。
# 负样本取错了，线自然划早了。
#
# 旁证：88 篇已发布笔记两两互比，**57 篇（65%）按 0.30 重审会被自己的同门拦下**，
# 其中包括全账号观看最高那篇（《晋升答辩被评委问倒怎么办》，1177 观看 / 31 赞 /
# 41 藏），它会以 55% 被拦。规则比账号自己实际发出去的东西严得多。
#
# ⚠️ 40%-68% 这一带在存量里还压着约 16 篇，真实状态没有逐篇核过 —— 那是真灰区。
...
def _published_titles() -> tuple:
    """(slug, 标题) —— **只取真发出去了的行**（发布列以 ✅ 开头）。slug 取自
    「成稿文件」列，跟 _other_draft_titles 用同一套键，这样同一篇稿子发布后
    再被 health_check 扫到，不会拿自己的发布记录跟自己比对。

    ⛔ 2026-09-16 加 ✅ 过滤。原来不管发布结果、只要「标题」列非空就收进比对池，
    于是一堆「⏸ 待人工定时」「❌ 点击失败」的失败重试行也在里面 —— 同一篇稿
    定时失败重试 5 次，它的标题就在池子里占 5 个位置去拦别人，而读者一次都没见过。
    """
    ...
    with PUB_LOG.open(encoding="utf-8-sig") as f:
        for r in csv.DictReader(f):
            ...
            if not title or not (r.get("发布") or "").strip().startswith("✅"):
                continue
...
def _other_draft_titles() -> tuple:
    """(slug, 标题) —— **只取素材库根目录的在库活稿，不含归档稿**。

    ⛔ 2026-09-16 把归档稿踢出比对池 —— 它构成一个自我加剧的棘轮：
    一篇稿因为标题撞车被毙 → 进归档稿 → 它的标题**永久留在池子里**继续拦后来者
    → 后来者也被毙、也进归档、也变成新的拦路石。实测池子 249 个标题里
    161 个（65%）是读者永远看不到的，其中 68 个来自归档稿。
    归档稿按定义是不会再发的稿，拿它拦活稿是纯粹的损失。

    在库活稿仍然留在池里：它们是「在飞行中」、随时可能发出去的，同一批次里
    两篇标题雷同的稿都放行才是真问题（闸门在批次开始时一次性算完，
    第二篇不会因为第一篇刚发出去就自动被拦）。
    """
    out = []
...
def check_one(d, f, all_drafts, lane_override=None):
    ...
    # 必须命中清单 第 3 条：CTA 相关检查
    #
    # ⛔ 2026-09-17：CTA 由**必须**降为**可选**（清单第 3 条整条重写，Eric 定）。
    # 旧规则连同它背后那条「每个选项必须有回了才拿得到的东西」是一起的 ——
    # 而那条明文要求把最常见的那类答案从正文里抽走留给评论区，跟评分卡维度 5
    # 「读者关掉页面时手上多了什么」直接对撞。删掉留一手，CTA 这个硬性要求
    # 也就没有理由继续当闸门了：没有真正的下一步时，硬凑一个互动钩子只是噪声。
    # 「没给编号选项」这条检查**整条删除**：CTA 可选之后它既不能拦发布，
    # check_one 又只有 issues 一个返回通道、没有 warn 通道，留着就是死代码。
    # 编号 CTA 仍是推荐形态（评论成本最低，写在清单第 3 条里），只是不再是闸门。
    # ⛔ 下面两条 CTA 红线**保留为 issue，不许一起删**：
    #   · CTA_EXPENSIVE（「说说你的情况」型）—— 它保护的是读者，公开贴领导原话有真实职场风险
    #   · 评论换资源的交换句式 —— 平台红线，账号 2026-08-20 已吃过一次警告
    # ⚠️ 这是一次**放宽**。规则收紧要跑 regress() 显性化存量债，放宽同样要跑一次：
    # 08-08 换 CTA 口径时废掉的那批存量稿，有一部分会因为这次放宽重新变成可发。
