...
LANE_SPEC = {
    "搜索流": {
        ...
        "rules": """- 标题公式：**[关键词] + [对方内幕 / 反常识事实 / 具体悬念]**，≤20 字。
  图是给人看的，标题是给机器看的 —— 关键词保证被搜到，悬念/内幕保证被点开。
  关键词**不必在最左、不必是完整搜索原句**；必须命中三项中的至少一项。
  ⛔ 四种不及格：纯搜索原句复读（「第三轮hr面试一般会问什么」）／
  只复述读者已知的困境（「汇报总被打断？」）／把答案在标题里说完（说完就没人点）／
  自夸承诺（「这5招绝了」）。
  ✅ 样板：「第三轮hr面试会问什么，只要准备这个答案就行」（悬念+低成本承诺）、
  「晋级答辩为啥老黄牛总被刷下去」（身份代入+反常识事实）。
  📊 依据 330 条搜索位笔记实测：写「对方在想什么」高互动组占 28%、低组 5%；
