...
LANE_SPEC = {
    "搜索流": {
        ...
        "rules": """- 标题公式：**[关键词] + [推翻读者笃信的一个预设]**，≤20 字。
  图是给人看的，标题是给机器看的 —— 关键词保证被搜到，反转+悬念保证被点开。
  关键词不必在最左、不必是完整搜索原句；但**必须否定读者脑子里已有的一个判断**。
  ✅ 样板（都在推翻一个预设）：
  「第三轮hr面试他不再看能力，改看这个」（推翻"第三轮考能力"）
  「汇报被领导打断，多半不是他没耐心」（推翻"打断=不耐烦"）
  「晋升答辩被问倒，评委问的根本不是项目」（推翻"答辩考项目"）
  「晋升答辩有可能不过，被刷的常是干最多的」（推翻"干得多自然能过"）
  常见载体是**对方内幕**：面试官/HR/大领导/评委那一侧在想什么、在看什么。
  ⛔ 五种作废：纯搜索原句复读／只复述读者已知的困境（「汇报总被打断？」）／
  **纯悬念不推翻认知**（「还有一句能救」「换成这句就行」——悬念党疲劳）／
  把答案在标题里说完／自夸承诺（「这5招绝了」）。
  悬念必须挂在被推翻的预设上：「改看这个」成立是因为前半句已否掉"看能力"。
  📊 330 条搜索位笔记实测：写「对方在想什么」高互动组 28%/低组 5%；
  写「你的困境」低组 35%/高组 2%；疑问句低组 55%/高组 20%；留悬念高组 15%/低组 0%。
