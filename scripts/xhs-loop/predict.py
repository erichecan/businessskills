...
#
# ⛔ 2026-09-14：按 预测校准.csv 的实测倍数重新校准（`预测校准.csv` 里「样本已达 N 篇，
# 可以考虑校准了」这句提示从 2026-08-14 起就一直打印，但没人真的改过这行——见
# docs/20260913-写稿策略进化路径.md Phase 1）。按关键词去重、只看「指标=观看」：
#   低 n=13 倍数中位 0.58 → 旧(250,600) 新(150,350)
#   中 n=11 倍数中位 0.17 → 旧(350,900) 新( 60,150)
#   高 n=16 倍数中位 0.33 → 旧(200,700) 新( 65,230)
# 中/高两档过去被系统性高估 3-6 倍，是这次唯一真正动了量级的改动。
VIEWS_BASE = {"低": (150, 350), "中": (60, 150), "高": (65, 230)}
...
LOW_SPLIT_LIKES = 20
# ⛔ 2026-09-14：检查过是否也该按上面同一批数据校准这两档，按「前排中位赞」拆开后
# 稀（<20）只剩 1 个独立样本、常（≥20）只有 4 个，都够不到 CALIB_MIN_SAMPLE=5，
# 拆完的子桶样本不够——维持原值，不在这里加第二次调整去凑不够的样本。
