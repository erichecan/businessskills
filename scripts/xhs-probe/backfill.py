...
# 阶段/场景/概念三列由 scene_map.py 打标写入，pick_topic 靠「场景」列做
# 【场景×产出缺口】选题（2026-08-18 c5c8139 引入）。这里必须列进来，否则这一轮
# 整表重写会把 scene_map.py 刚打的标悄悄清空——2026-09-12 查出来的实际情况就是
# 这三列在词库.csv 里从未真正落地过，pick_topic 因此每次都读到空值，缺口逻辑
# 形同虚设，一路退回全局兜底（旧的纯贪心排序），选题偏斜的根因就在这里。
CIKU_COLS = ["关键词", "场景域", "场景类型", "阶段", "场景", "概念", "意图强度", "竞争密度",
             "人工初判", "关联案例ID", "状态", "发布日", "笔记链接", "搜索来源占比", "备注"]
